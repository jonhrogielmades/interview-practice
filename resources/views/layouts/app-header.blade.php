<header
    class="sticky top-0 z-99999 flex w-full border-b border-gray-200/50 bg-white/80 backdrop-blur-2xl transition-all duration-300 dark:border-white/5 dark:bg-gray-900/80"
    x-data="{
        ...
        initDateTime() {
            const updateTime = () => {
                const now = new Date();
                this.currentTime = now.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true });
    ...
    <div class="flex flex-col items-center justify-between grow xl:flex-row xl:px-6">
        <div
            class="flex min-w-0 items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200/50 dark:border-white/5 sm:gap-4 xl:justify-normal xl:border-b-0 xl:px-0 lg:py-4">
        ...
        <div :class="isApplicationMenuOpen ? 'flex' : 'hidden'"
            class="w-full flex-col items-stretch gap-4 px-4 py-4 shadow-theme-md sm:px-5 xl:flex xl:flex-row xl:items-center xl:justify-end xl:px-0 xl:shadow-none">
            
            <div class="flex items-center gap-2 2xsm:gap-3">
                
                <!-- Date and Time -->
                <div class="flex flex-col items-end justify-center mr-2 text-right">
                    <span class="text-sm font-semibold text-gray-900 dark:text-white/90" x-text="currentTime"></span>
                    <span class="text-[11px] font-medium text-gray-500 dark:text-gray-400" x-text="currentDate"></span>
                </div>

                <!-- Language Selector -->
                <div x-data="{ 
                        isOpen: false, 
                        currentLang: localStorage.getItem('app_locale') || 'EN',
                        languages: {
                            'EN': 'English',
                            'PH': 'Filipino',
                            'BIS': 'Bisaya'
                        ...
                        setLanguage(code) {
                            ...
                            if (window.AITranslator) {
                                window.AITranslator.translatePage(code);
                            }
                        }
                    }" class="relative flex">
                    <button @click="isOpen = !isOpen" @click.outside="isOpen = false" class="flex h-11 items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 shadow-theme-xs transition hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900 dark:hover:bg-white/[0.03]">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-gray-500 dark:text-gray-400">
                            <circle cx="12" cy="12" r="10"></circle>
                            <line x1="2" y1="12" x2="22" y2="12"></line>
                            <path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path>
                        </svg>
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300" x-text="currentLang"></span>
                        <svg class="h-4 w-4 text-gray-400 transition-transform duration-200" :class="isOpen ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    <!-- Dropdown -->
                    <div x-show="isOpen" x-transition class="absolute right-0 top-full mt-2 w-40 rounded-xl border border-gray-200 bg-white py-2 shadow-theme-lg dark:border-gray-800 dark:bg-gray-900 z-50">
                        <template x-for="(name, code) in languages" :key="code">
                            <button @click="setLanguage(code)" class="flex w-full items-center justify-between px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 hover:text-brand-600 dark:text-gray-300 dark:hover:bg-white/[0.03] dark:hover:text-brand-400">
                                <span x-text="`${name}`"></span>
                                <span class="text-xs text-gray-400" x-text="code"></span>
                            </button>
                        </template>
                    </div>
                </div>
                ...
                <div class="flex h-11 items-center gap-1 rounded-lg border border-gray-200 bg-white px-1.5 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900"
                    role="group" aria-label="Theme mode">
                    <button
                        type="button"
                        ...
                        :class="$store.theme.theme === 'light' ? 'border-brand-500 bg-brand-50 text-brand-600 dark:border-brand-500/30 dark:bg-brand-500/10 dark:text-brand-400' : 'border-transparent'"
                    ...
                    <button
                        type="button"
                        class="flex h-8 w-8 items-center justify-center rounded-lg border text-gray-500 transition hover:bg-gray-100 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:text-gray-400 dark:hover:bg-gray-800"
                        :class="$store.theme.theme === 'dark' ? 'border-brand-500 bg-brand-50 text-brand-600 dark:border-brand-500/30 dark:bg-brand-500/10 dark:text-brand-400' : 'border-transparent'"
                ...
                <button
                    ...
                    class="flex h-11 w-11 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 shadow-theme-xs transition hover:bg-gray-100 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800"
                    aria-label="Toggle Fullscreen"
                    :title="isFullscreen ? 'Exit Fullscreen' : 'Enter Fullscreen'">
                    <svg x-show="!isFullscreen" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3"></path>
                    </svg>
                    <svg x-show="isFullscreen" style="display: none;" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
